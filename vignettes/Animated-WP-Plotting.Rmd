...
To build the labels, we're going to use a custom function to pull the player names out of the play text. [Saiem Gilani](https://twitter.com/SaiemGilani) wrote the bulk of the regular expressions here to get the names. EDIT: The improved version of this is now included in the ```cfb_pbp_data()``` function by default as of version 1.0.3.


Now we are ready to transform our play-by-play data frame to match what we need to work with Lee Sharpe's animation code. First, we will rename several columns that are similar between ```cfbscrapR``` and ```nflfastR``` to match ```nflfastR```'s column names. Then we need to create a few new columns that ```cfbscrapR``` doesn't have. `game_seconds_remaining` is really just renaming `TimeSecsRem` but correcting for the half. `result` grabs from the game information and is the difference in the home and away score. We create the `time` column to use as the clock on the right side of the animation. 
