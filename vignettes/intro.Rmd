...
To use the packages, we need to do two things. First, one time only on your machine, you need to install the packages. Then, every time you open up R Studio, you will need to load the packages.
...
Don't worry about what all that means for now. Now, after running those lines of code, you have all the packages you need to get started with college football analytics. Let's load the packages and download the play-by-play data for 2019. First we load the three necessary packages, then we're going to run a loop over the number of weeks in the season and use the ```cfb_pbp_data()``` function, which pulls the play-by-play.
...
# Using the [Data Repository](https://github.com/saiemgilani/cfbscrapR-data)
...
# Using the ```cfb_pbp_data()``` function
```{r cfb_pbp_data, message=FALSE,warning=FALSE,eval=FALSE}
week_vector = 1:15
year_vector = 2019
...
```
This code will take a few minutes (~8:10) to run - it's pulling a lot of data from the API at once. Give your R console a break until it finishes! You'll know it's done running when you can see ```pbp_2019``` in the "Environment" window in the top right of your R console (and when the little caret `>` pops up in your terminal, at the bottom left). 
...
Let's pull the game data, using the ```cfb_game_info()``` function, and merge the two. Then, we'll make use of the ```dplyr::filter``` and ```dplyr::mutate``` functions from the ```tidyverse```. 
...
Above, we pulled 2019 game data from the API into an object called ```games_19```, and then we used ```dplyr::left_join``` to merge each play to the information of the game it happened in, an object called ```plays19```. We matched the plays to the games by telling R that the column ```game_id``` in ```pbp_2019``` was the same as the column ```id``` in the ```games_19``` object. 
...
Now we have a clean dataset of every rush and pass that happened in 2019. (Quick note: unfortunately, due to the way the pbp data is generated on ESPN, QB scrambles from dropbacks are coded as rushes. It's not ideal, but it's not a deal-breaker; just something to remember as you do your analysis).
...
Ok, now let's play with the tidyverse a little more. We've used ```dplyr::filter()```, which is an extremely useful function. The next most useful functions actually do things to the data: ```dplyr::group_by()```, and ```dplyr::summarise()```.
...
Now we can ask and answer some fun questions. Who had the best rushing offense, on a per play basis? Who had the best passing offense, on a per-play basis? We will do this using the ```dplyr::arrange(desc())``` function, which tells R to order the data from greatest to least (descending).
...
In analytics, we often want to break down situations. Let's use ```dplyr::filter``` to select only first down plays and see which offenses performed the best on first downs only:
...
Lastly, let's do an example of creating some rates and using some advanced filters: We'll create success rate on early downs and early downs rush rate, filtering out garbage time. We'll use the same filter, summarise, and mean functions as above!
...
We have some pretty data, now let's learn how to make a basic ```ggplot2::ggplot()``` to visualize our data! We didn't have to load ```ggplot2::ggplot()```, it's included in the tidyverse. We'll take the success data from above, and start with a basic plot, then add some fancy formatting.

The syntax here is pretty simple: we will grab our dataset and use the pipe to say "open a plot". Then we'll tell the plot what the x and y coordinates are, then indicate the markers, in this case a scatterplot with ```ggplot2::geom_point()```. Notice that we use ```+``` instead of ```%>%``` once we've called ```ggplot2::ggplot()```. The syntax here tells R to take the ```success``` object and open a plot with the x.axis ```rush.rte``` and the y.axis ```success.rte```, PLUS all these features. That can be a little confusing to keep straightforward, but R will tell you what's going on with an error if you mix that up!

```{r successplot, message=FALSE,warning=FALSE}
success %>% ggplot(aes(x=rush.rte, y=success.rte)) + geom_point()
```

Ok, that's pretty cool! Let's dress it up a little bit by adding a title and labeling the axes, using the ```ggplot2::labs()``` option. We will also throw a couple reference lines - a vertical and horizontal line indicating the mean values of each statistic to help us compare. Plus, here are some tweaks I like that make the graph look a little better (all the ```ggplot2::theme()``` stuff).
...
We'll make use of our graph from above, telling the plot that we want to use the logo column of chartdata to identify teams instead of a regular ol' marker. This plot takes a minute to run - this is a consequence of pulling the logos from the web each time. It's not ideal, but again this is the easiest way to make these graphs, so I opted for simplicity.
...
You can save the graphs with ```ggplot2::ggsave()```. That gives you a nice, high-res png file that we can post to Twitter or put in a blog post: ```ggplot2::ggsave('file.png', height = 7, width = 13, dpi = 300)```
...
I added a couple fun features here: a **```ggplot2::geom_hline()```** reference line at zero, to help us better understand the graph, and a subtitle to clarify what was on the graph. Notice that TCU's EPA was spread almost evenly across the field, but you can see some serious negative outliers that weighed them down. We could call those outliers into our console to explore them more:
...
Lastly, let's explore the ```gt``` package to list the top ten rushing and passing offenses and defenses in the country. We will take our dataset, sort it by the value we want, add a rank variable using ```dplyr::mutate()```, then apply ```gt::gt()```, simply enough.
...
This is just a little taste of what you can do in college football data, thanks to [CollegeFootballData.com](collegefootballdata.com) and [cfbscrapR](https://twitter.com/cfbscrapR). Please tag our twitter account whenever you publish a graphic or analysis using ```cfbscrapR``` so that we can see it and retweet it!
...
* [```cfb_pbp_data()```](https://saiemgilani.github.io/cfbscrapR/reference/cfb_pbp_data.html) gets you the play by play for a given week or team. Can loop over this to pull the whole season. 
* [```cfb_game_info()```](https://saiemgilani.github.io/cfbscrapR/reference/cfb_game_info.html) gets you the home team, away team, game score, and more info about games.  
* [```plot_pbp_sequencing()```](https://saiemgilani.github.io/cfbscrapR/reference/plot_pbp_sequencing.html) and [```plot_wpa()```](https://saiemgilani.github.io/cfbscrapR/reference/plot_wpa.html) are both built-in graphics you can play with. See documentation.   

[```tidyverse```](https://www.tidyverse.org/): the set of R data science packages that help you wrangle and analyze data.  

### Functions we used in the tidyverse (and other useful base R functions)

* [```dplyr::mutate()```](https://dplyr.tidyverse.org/reference/mutate.html): creates new variables  
* [```dplyr::group_by()```](https://dplyr.tidyverse.org/reference/group_by.html) and [```dplyr::summarise()```](https://dplyr.tidyverse.org/reference/summarise.html): groups observation and calculates summary statistics  
* [```dplyr::filter()```](https://dplyr.tidyverse.org/reference/filter.html): keeps observations based on certain conditions (remember to use ```==```)  
* [```dplyr::left_join()```](https://dplyr.tidyverse.org/reference/filter.html): takes one dataframe and merges it with another  
* [```ggplot2::ggplot()```](https://ggplot2.tidyverse.org/reference/ggplot.html) and [```ggplot2::geom_point()```](https://ggplot2.tidyverse.org/reference/geom_point.html): graphing functions that set up a graph and plots scatter plots  
* [```head()```](http://search.r-project.org/R/library/utils/html/head.html), [```tail()```](http://search.r-project.org/R/library/utils/html/head.html), [```dplyr::glimpse()```](https://tibble.tidyverse.org/reference/glimpse.html) and [```levels()```](https://stat.ethz.ch/R-manual/R-patched/library/base/html/levels.html): all help you inspect the data  

[```gt```](https://gt.rstudio.com/index.html): a nice package for getting publication-quality tables  
See [Tom's Cookbook](https://jthomasmock.github.io/nfl_plotting_cookbook/) to help tweak [```gt```](https://gt.rstudio.com/index.html) with all sorts of features.  
